...
{
    resize(850, 550);
    setWindowTitle(tr("BatCoin") + " - " + tr("Wallet"));
#ifndef Q_WS_MAC
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("BatCoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("BatCoin client"));
...
void BitcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
    ...
    {
        // Click on system tray icon triggers "show/hide BatCoin"
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to BatCoin network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid BatCoin address or malformed URI parameters."));
    }
